...
gulp.task('pl-sass', function() {
  return gulp.src(path.resolve(paths().source.css, '**/*.scss'))
    ...
    .pipe(autoprefixer({remove: false, cascade: false}))
    .pipe(gulp.dest(function(file){
      //flatten anything inside the styleguide into a single output dir per http://stackoverflow.com/a/34317320/1790362
      file.path = path.join(file.base, path.basename(file.path));
      return resolvePath(path.join(paths().source.css));
    }))
    // .pipe(rename({
    //   suffix: ".min"
    // }))
...
function watch() {
  gulp.watch(resolvePath(paths().source.root) + '/**/*.scss').on('change', gulp.series('pl-sass', reloadCSS));
  gulp.watch(resolvePath(paths().source.js) + '/**/*.js').on('change', gulp.series('pl-copy:js', reloadCSS));
